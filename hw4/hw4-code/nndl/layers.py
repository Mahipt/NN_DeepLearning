...
def affine_forward(x, w, b):
  ...
  # Reshape x into a N by D input 
  x_reshape = np.reshape(x, (N, D))
  b_reshape = np.reshape(b, (1, M))
  out = np.dot(w, x_reshape) + b_reshape
  cache = (x, w, b)
...
def affine_backward(dout, cache):
  ...
  # during forward it is X_reshape @ w 
  # Therefore after dervative by X it would left w.T  
  dx_reshape = np.dot(dout, w.T)
  dx = np.reshape(dx_reshape, x.shape) 
  dw = np.dot(dx_reshape.T, dout)
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    x_normalize = (x - minibatch_mean) / np.sqrt((minibatch_var**2) + eps)
    out = gamma * x_normalize + beta

    
    
    # Using Adam Algorithm to update 
